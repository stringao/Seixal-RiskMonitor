...
public sealed record GetAlertHistoryQuery(
    int Page,
    int PageSize,
    AlertSeverity? Severity,
    string? CacheKey) : IQuery<AlertListResponse>;

public sealed class GetAlertHistoryHandler(
    GeoRiskDbContext db,
    ICacheService cache) : IQueryHandler<GetAlertHistoryQuery, AlertListResponse>
{
    private static readonly TimeSpan CacheTtl = TimeSpan.FromMinutes(2);

    public async Task<AlertListResponse> HandleAsync(GetAlertHistoryQuery query, CancellationToken ct)
    {
...
public static class GetAlertHistoryEndpoint
{
    public static RouteGroupBuilder MapGetAlertHistory(this RouteGroupBuilder group)
    {
        group.MapGet("/history", async (
            int? page, int? pageSize, AlertSeverity? severity,
            IQueryHandler<GetAlertHistoryQuery, AlertListResponse> handler) =>
        {
            var p = page ?? 1;
            var ps = pageSize ?? 50;
            var cacheKey = $"alert-history:{severity}:{p}:{ps}";
            var result = await handler.HandleAsync(new GetAlertHistoryQuery(p, ps, severity, cacheKey), default);
